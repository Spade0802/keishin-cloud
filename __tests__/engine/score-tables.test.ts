...
describe('calculateTechStaffValues via convertLegacyStaffToExtracted（別紙二→業種別技術職員数値）', () => {
  /** Helper: legacy format -> canonical industryTotals */
  function calcLegacy(staff: Array<{
    ...
    const withNames = staff.map((s, i) => ({ ...s, name: `staff-${i}` }));
    const extracted = convertLegacyStaffToExtracted(withNames);
    return calculateTechStaffValues(extracted).industryTotals;
  }
